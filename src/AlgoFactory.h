...
#pragma region Factory-Boilerplate
	...
	class Factory
	{
	  public:
		template < class... T >
		static Base *make( const QString &s, T &&...args )
		{
			auto x = Factory::data().find( s );
			if ( x != Factory::data().end() ) return ( *x )( std::forward< T >( args )... );
			else return nullptr;
		}
...
#pragma region AlgoFactory
	...
	class QDESIGNER_WIDGET_EXPORT AlgoFactory
		: public QWidget
		, public Ui::AlgoFactory
	{
		Q_OBJECT
	  ...
	  public:
		...
		AlgoFactory( QWidget *parent = nullptr );
		~AlgoFactory();
