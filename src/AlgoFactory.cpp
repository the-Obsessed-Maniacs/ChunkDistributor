...
namespace Algo
{
	...
	AlgoFactory::AlgoFactory( QWidget *pw )
		: QWidget( pw )
		, idleProc( new QChronoTimer( this ) )
	{
		...
		connect( sb_prio_cnt, &QSpinBox::valueChanged, this, &AlgoFactory::setNPrio );
		...
		connect( idleProc, &QChronoTimer::timeout, this, &AlgoFactory::idleProcessing );
	}
	AlgoFactory::~AlgoFactory()
	{
		if ( worker )
			if ( auto t = worker->thread() ) t->quit(), t->wait();
	}
	bool AlgoFactory::eventFilter( QObject *o, QEvent *e )
	{
	...
	void AlgoFactory::setNPrio( int n )
	{
	...
	void AlgoFactory::setShowAniCfg( bool sa )
	{
		if ( sa != showAniCfg )
		{
	...
	void AlgoFactory::setProgressReporting( ProgressDisplay as )
	{
		if ( prgMode != as )
		{
			prgMode = as;
	...
	void AlgoFactory::on_sl_aniDur_valueChanged( int nms )
	{
		PositionAni::duration = nms;
	}
	void AlgoFactory::on_bg_visMode_idPressed( int button_id )
	{
	...
	void AlgoFactory::on_pb_simulate_clicked()
	{
		...
		{ // Der Algo läuft schon - hier geht's jetzt um's Pausieren/Depausieren...
			...
			sc->addItem( gfx = new AlgoGfx( v->contentsRect().size(),
											*reinterpret_cast< QList< quint32 > * >( &chunks ),
											*reinterpret_cast< QList< quint64 > * >( &pages ) ) );
			v->setScene( sc );
			...
			auto runnerThread = new QThread( this );
			if ( worker = AlgoWorker::make( cb_algo->currentText() ) )
			{
				worker->setInitData( chunks, pages );
				connect( worker, &WorkerBase::final_solution, this, &AlgoFactory::finalSolution,
						 Qt::QueuedConnection );
				connect( worker, &WorkerBase::state_changed, this, &AlgoFactory::stateChange,
						 Qt::QueuedConnection );
				connect( worker, &WorkerBase::page_finished, this, &AlgoFactory::pageFinished,
						 Qt::QueuedConnection );
				connect( this, &AlgoFactory::do_continue, worker, &WorkerBase::iterate,
						 Qt::QueuedConnection );
				connect( this, &AlgoFactory::do_pause, worker, &WorkerBase::pause,
						 Qt::QueuedConnection );
				worker->moveToThread( runnerThread );
				runnerThread->start( QThread::LowPriority );
				pb_generate->setEnabled( false );
				cb_algo->setEnabled( false );
				connect( pb_simulate, &QPushButton::clicked, this, &AlgoFactory::do_pause );
				emit do_continue();
	...
	void AlgoFactory::stateChange( State st )
	{
		if ( st != lastSt )
		{
	...
	void AlgoFactory::pageFinished( quint16 page_id )
	{
		updateRequests.insert( page_id );
		if ( prgMode != silent && !idleProc->isActive() ) idleProc->start();
	}
	void AlgoFactory::finalSolution( quint64 iteration_count, quint64 selection_count,
									 quint64 deselection_count, qreal results_good )
	{
		...
		emit stateMessageChanged( msg );
		disconnect( pb_simulate, &QPushButton::clicked, this, &AlgoFactory::do_pause );
		pb_generate->setEnabled( true );
		cb_algo->setEnabled( true );
	}
	void AlgoFactory::idleProcessing()
	{
	...
	QPair< quint16, quint16 > AlgoFactory::read_page_info( const QString tx ) const
	{
		auto tl = tx.split( '+' );
		auto sa = read_maybehex( tl.first() );
		return { sa, tl.count() > 1 ? read_maybehex( tl.last() ) : ( ( sa + 256 ) & 0xff00 ) - sa };
	}
	quint16 AlgoFactory::read_maybehex( const QString tx ) const
	{
		auto tt	 = tx.trimmed();
